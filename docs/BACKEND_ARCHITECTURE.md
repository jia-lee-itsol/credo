...
**마지막 업데이트**: 2025-12-17 (성당별 알림 그룹화, 아코디언 기능 추가)
...
- ✅ Firebase Storage (이미지 및 PDF 파일 저장) - **구현 완료**
  - 이미지 업로드 기능 구현 완료
  - PDF 파일 업로드 기능 구현 완료 (최대 10MB)
  - 파일 경로: `posts/{postId}/` 또는 `posts/{postId}/comments/{commentId}/`
...
3. **커뮤니티**: 게시글, 댓글, 좋아요 - **구현 완료**
   ...
   - PDF 파일 업로드 및 뷰어 구현 완료 (게시글: 최대 2개, 댓글: 최대 1개)
   - 공식 공지 vs 커뮤니티 게시글 구분 구현 완료
   - 알림 타입 구분 (새 글, 코멘트, 공지사항) 구현 완료
...
└──────────────┘  └──────────────┘  └──────────────┘
        ...
        ▼                 ▼                 ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│ Firebase     │  │ Google Maps  │  │ OpenAI       │
│ Storage      │  │ Geocoding    │  │ GPT API      │
│              │  │ API          │  │              │
│ - 이미지     │  │              │  │ - 묵상 가이드 │
│ - PDF 파일   │  │ - 주소→좌표  │  │ - 축하 메시지│
└──────────────┘  └──────────────┘  └──────────────┘
